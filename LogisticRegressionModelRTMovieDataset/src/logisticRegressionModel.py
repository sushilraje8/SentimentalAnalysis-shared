...
def prepare_dictionary():
    ...
    if len(dictionary) is 0:
        ...
        dictionary = [stemmer.stem_word(word.rstrip("\n")) for word in get_file_lines("/home/tsingh/git/sushilraje8/SentimentalAnalysis-shared/python/dictionary/tempDictionaryn.txt")]
...
def prepare_data():
    ...
    if len(X) is 1 or len(y) is not 5:
        ...
        train_data = get_file_lines("/home/tsingh/git/sushilraje8/SentimentalAnalysis-shared/python/data/trainML.tsv")
...
def predict2(test_data):
    ...
    print ("len(X), len(y)"), len(X), len(y)
    cnt = 0
    y_final = logreg.predict(X)
    print ("y_final"), y_final
    y_ffinal = y_final.tolist()
    i = 0
    for value in y_ffinal:
        if (y[0][i] == 1):
    ...
    print ("Accuracy = "), (cnt*1.0)/y[0].count(1)
...
def main(re_train):
    ...
    prepare_data()
    train(0)    
    test_data = get_file_lines("/home/tsingh/git/sushilraje8/SentimentalAnalysis-shared/python/data/test.tsv")
    predict2(test_data)
